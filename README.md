...
2. [Vox-Box](https://github.com/gpustack/vox-box): Basically `pip install vox-box`. After installation, run `vox-box start --huggingface-repo-id Systran/faster-whisper-small --data-dir ./ --host localhost --port 8099` and close the server.  Test the basic utility via curl before using inside Obsidian e.g.
```bash
